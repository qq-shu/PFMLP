...
class BaseMultilayerPerceptron(BaseEstimator, metaclass=ABCMeta):
    ...
    def _initialize(self, y, layer_units):
        ...
        # print('self.coefs_')
        # print(type(self.coefs_))
        # np.save('coef_init.npy',self.coefs_)
        # # string_coef = json.dumps(self.coefs_)
        # # with open('coef_.txt','w') as fout:
        # #     fout.write(string_coef)
        # # print(self.coefs_)
        # print('self.intercepts_')
        # print(type(self.coefs_))
        # np.save('intercept_init.npy',self.intercepts_)
        # string_intercept = json.dumps(self.intercepts_)
        # with open('intercepts_.txt','w') as fout:
        #     fout.write(string_intercept)
        # print(self.intercepts_)
    ...
    def _fit(self, X, y, incremental=False):
        ...
        coef_grads = [np.empty((n_fan_in_, n_fan_out_)) for n_fan_in_,
                      n_fan_out_ in zip(layer_units[:-1],
                                        layer_units[1:])]
        np.save('C:\\Users\\FangHaokun\\PycharmProjects\\ml\\1209\\mid_need\\coef_2.npy',coef_grads)

        intercept_grads = [np.empty(n_fan_out_) for n_fan_out_ in
                           layer_units[1:]]
        np.save('C:\\Users\\FangHaokun\\PycharmProjects\\ml\\1209\\mid_need\\intercept_2.npy',intercept_grads)
        ...
        try:
            for it in range(self.max_iter):
                ...
                for batch_slice in gen_batches(n_samples, batch_size):
                    ...
                    output = open('mid_need/grad2.pkl', 'wb')
                    pickle.dump(intercept_grads, output)
                    output.close()
